...
function getRectangleString(width, height) {
    ...
    const firstRow = (_, col) => row(col, '┌', '┐', '─');
    const normalRow = (_, col) => row(col, '│', '│', ' ');
    const lastRow = (_, col) => row(col, '└', '┘', '─');
    return Array(height).fill().map((_, row)=> {
            if (row === 0) {
                return Array(width).fill().map(firstRow).join('')
            } else if (row === height - 1) {
                return Array(width).fill().map(lastRow).join('')
            } else {
                return Array(width).fill().map(normalRow).join('')
            }
        }).join('\n') + '\n';
...
function encodeToRot13(str) {
    return str.replace(/[a-z]/gi, c => String.fromCharCode((c <= "Z" ? 90 : 122) >= (c = c.charCodeAt(0) + 13) ? c : c - 26));
}
...
function getCardId(value) {
    ...
    const cardValue = (k) => card[k] ? card[k]() : +k - 1;
    var suit = {
        '♣': 0,
        '♦': 13,
        '♥': 26,
    ...
    return cardValue(first) + suit[last]
}
